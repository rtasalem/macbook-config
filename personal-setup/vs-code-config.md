...
- [ ] [Prettier](https://marketplace.visualstudio.com/items?itemName=esbenp.prettier-vscode) - within the `settings.json` ensure to set the following configuration for Prettier: `"prettier.singleQuote": true` and `"pretter.semi": false`. This means that when formatting, by default Prettier will wrap strings in single quotes rather than double quotes (look out for this syntax with languages that distinguish between single and double quotes!). Also stops semicolons from being added to the end of each line of code by default (again, consider the syntax of the language being used and whether it requires semicolons e.g. Java).
